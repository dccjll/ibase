...
import com.dcc.ibase.network.api.ApiService;
import com.dcc.ibase.network.callback.ACallback;
import com.dcc.ibase.network.VHttpStatics;
import com.dcc.ibase.network.func.ApiFunc;
import com.dcc.ibase.network.func.ApiRetryFunc;
import com.dcc.ibase.network.mode.ApiHost;
import com.dcc.ibase.network.mode.CacheMode;
...
public abstract class BaseHttpRequest<R extends BaseHttpRequest> extends BaseRequest<R> {
    ...
    protected void generateLocalConfig() {
        ...
        if (sVHttpGlobalConfig.getGlobalParams() != null) {
            params.putAll(sVHttpGlobalConfig.getGlobalParams());
        }
        if (retryCount <= 0) {
            retryCount = sVHttpGlobalConfig.getRetryCount();
        }
        if (retryDelayMillis <= 0) {
            retryDelayMillis = sVHttpGlobalConfig.getRetryDelayMillis();
        }
        if (isLocalCache) {
            if (cacheKey != null) {
                VHttp.getApiCacheBuilder().cacheKey(cacheKey);
            } else {
                VHttp.getApiCacheBuilder().cacheKey(ApiHost.getHost());
            }
            if (cacheTime > 0) {
                VHttp.getApiCacheBuilder().cacheTime(cacheTime);
            } else {
                VHttp.getApiCacheBuilder().cacheTime(VHttpStatics.CACHE_NEVER_EXPIRE);
            }
        }
        if (baseUrl != null && isLocalCache && cacheKey == null) {
            VHttp.getApiCacheBuilder().cacheKey(baseUrl);
        }
