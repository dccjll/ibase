...
import com.dcc.ibase.utils.MD5Utils;
...
public class DiskCache implements ICache {
    ...
    private long cacheTime = VHttpStatics.CACHE_NEVER_EXPIRE;

    public DiskCache(Context context) {
        this(context, getDiskCacheDir(context, VHttpStatics.CACHE_DISK_DIR),
                calculateDiskCacheSize(getDiskCacheDir(context, VHttpStatics.CACHE_DISK_DIR)));
    }
    ...
    public DiskCache(Context context, File diskDir, long diskMaxSize) {
        ...
        try {
            cache = DiskLruCache.open(diskDir, AppUtils.Companion.getVersionCode(context), 1, diskMaxSize);
        } catch (IOException e) {
            e.printStackTrace();
            LogManager.Companion.e(e);
        }
    }
            ...
            if (snapshot != null) {
                ...
                if (!TextUtils.isEmpty(content)) {
                    ...
                    if ((createTime + expireMills > Calendar.getInstance().getTimeInMillis())
                            || expireMills == VHttpStatics.CACHE_NEVER_EXPIRE) {
    ...
    private String getMd5Key(String key) {
        return MD5Utils.INSTANCE.getMessageDigest(key.getBytes());
    }
